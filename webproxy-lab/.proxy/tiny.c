...
static char reshttp[MAXLINE] = "1.0";
void doit(int fd);
...
void doit(int fd)
{
  ...
  if (!Rio_readlineb(&rio, buf, MAXLINE))
    return; // request를 읽는다. doit:readrequest
  ...
  { // request method 종류가 GET인지 비교한다. doit:beginrequesterr
  ...
  { // 파일이 디스크 상에 존재하지 않을 때. doit:beginnotfound
...
void serve_static(int fd, char *filename, int filesize, char *method)
{
  ...
  // 숙제 11.9: malloc, rio_readn, rio_writen 사용

  if (strcasecmp(method, "GET") == 0)
  { // method가 GET일때만 응답 객체 받게끔
    /* 요청한 파일의 내용을 fd로 복사해서 응답 본체를 보낸다. Send response body to client */
    srcfd = Open(filename, O_RDONLY, 0); // 읽기 위해 filename을 오픈한다.
...
void serve_dynamic(int fd, char *filename, char *cgiargs, char *method)
{
  ...
  if (Fork() == 0)
  {
    ...
    setenv("REQUEST_METHOD", method, 1);                                                                              // GET, HEAD                                                                         // REQEUST_METHOD 환경 변수를 요청 URI의 CGI 인자들로 초기화한다. servedynamic:setenv
